...
<div class="flex flex-col gap-4 max-w-4xl">
	...
	<div class="flex items-center gap-1.5 border-b border-themed pb-2 text-xs overflow-x-auto no-scrollbar">
		...
		<a href="/settings/alerts" class="flex items-center gap-1.5 rounded-md bg-indigo-600 px-3 py-1.5 font-semibold text-white shadow-sm whitespace-nowrap shrink-0">
			<Shield size={13} />
			<span>Alerts</span>
		</a>
					...
					<div class="flex items-start sm:items-center gap-3">
						...
						<div class="flex flex-col">
							...
							<div class="flex flex-wrap items-center gap-2 mt-1 text-[11px] text-label font-mono">
								<span>Condition:</span>
								<span class="text-indigo-300 font-semibold">{alert.metric}</span>
								<span>{alert.condition === 'greater_than' ? '>' : '<'}</span>
								<span class="text-indigo-300 font-semibold">{alert.threshold.toLocaleString()}</span>
